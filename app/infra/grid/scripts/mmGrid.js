app.directive('mmGrid', [function() {
    return {
        ...
        replace: false,
        transclude: false,
        controller: ['$scope', '$attrs', 'enums', '$filter', '$parse', '$timeout', '$element', 'mmAlertService', '$state', 'mmRest', 'mmMediaTypeIconService','mmUtils',
            function($scope, $attrs, enums, $filter, $parse, $timeout, $element, mmAlertService, $state, mmRest, mmMediaTypeIconService,mmUtils) {
                ...
                function mapColumnDefsProperties() {
                    ...
                    for (var index = 0; index < $scope.cols.length; index++) {
                        ...
                        $scope.getIconForAsset = function(mediaType) {
                            return mmMediaTypeIconService.getIconClassFormMediaType(mediaType);
                        }
              ...
              case enums.gridControlType.getName(selectList):
                ...
                if (typeof item.minWidth == "undefined")
                  item.minWidth = 150;
                celltemplate = '<div class="ngCellText" ng-class="col.colIndex()"><span ng-cell-text>{{selectNameFromId(col, row).val}}</span></div>';
                ...
                if (typeof item.listDataArray != "undefined") {
                  ...
                  if (typeof item.gridSelectSearch != "undefined" && item.gridSelectSearch) {
                    editcelltemplate = '<div ng-if="row.entity[\'rowLocked\']" class="ngCellText" ng-class="col.colIndex()"><span ng-cell-text>{{selectNameFromId(col, row).val}}</span></div><div ng-if="!row.entity[\'rowLocked\']"class="gridDropdownList" ng-style="{width:col.width}"><mm-drop-down mm-id="' + ddId + '" in-grid="true" ng-style="{width:col.width}" mm-show-label="false" mm-data-model="col.colDef.listDataArray" mm-label-width="0" mm-custom-control-width="{{col.width}}" mm-model="row.entity[col.field]" mm-layout-type="custom" mm-is-required="false" mm-custom-label-padding="0" mm-disable ="false" mm-error="error" mm-select-change-func="cellFunctionToExecuteForSelectListChange(row,col,row.entity[col.field])" show-search-box></mm-drop-down></div>';
                  } else {
                  ...
                  if (typeof item.gridSelectSearch != "undefined" && item.gridSelectSearch) {
                    editcelltemplate = '<div ng-if="row.entity[\'rowLocked\']" class="ngCellText" ng-class="col.colIndex()"><span ng-cell-text>{{selectNameFromId(col, row).val}}</span></div><div ng-if="!row.entity[\'rowLocked\']" class="gridDropdownList" ng-style="{width:col.width}"><mm-drop-down mm-id="' + ddId + '" in-grid="true" ng-style="{width:col.width}" mm-show-label="false" mm-data-model="row.entity[\'gridListDataArray\']" mm-label-width="0" mm-custom-control-width="{{col.width}}" mm-model="row.entity[col.field]" mm-layout-type="custom" mm-is-required="false" mm-custom-label-padding="0" mm-disable ="false" mm-error="error" mm-select-change-func="cellFunctionToExecuteForSelectListChange(row,col,row.entity[col.field])" show-search-box></mm-drop-down></div>';
                  } else {
                    editcelltemplate = '<div ng-if="row.entity[\'rowLocked\']" class="ngCellText" ng-class="col.colIndex()"><span ng-cell-text>{{selectNameFromId(col, row).val}}</span></div><div ng-if="!row.entity[\'rowLocked\']" class="gridDropdownList" ng-style="{width:col.width}"><mm-drop-down mm-id="' + ddId + '" in-grid="true" ng-style="{width:col.width}" mm-show-label="false" mm-data-model="row.entity[\'gridListDataArray\']" mm-label-width="0" mm-custom-control-width="{{col.width}}" mm-model="row.entity[col.field]" mm-layout-type="custom" mm-is-required="false" mm-custom-label-padding="0" mm-disable ="false" mm-error="error" mm-select-change-func="cellFunctionToExecuteForSelectListChange(row,col,row.entity[col.field])"></mm-drop-down></div>';
                  }
                }
              ...
              case enums.gridControlType.getName(searchableList):
                ...
                break;
              case enums.gridControlType.getName(assetIcon):

                                item['cellTemplate'] = '<div class="ngCellText" style="width:100px" ng-class="col.colIndex()"><i class="{{ getIconForAsset(row.entity.mediaType)}}" /></div>';
                            ...
                            case enums.gridControlType.getName(link):
                                ...
                                celltemplate = '<div ng-class="col.colIndex()" class="textBoxControl linkControl">' +
                ...
                $scope.getImageSrc = function(field) {
                    ...
                    //if not image - show default image
                    if (!mmUtils.utilities.IsImageUrl(field)) {
                        return './ignoreImages/default_image_small.png';
                    }
                ...
                $scope.showPreview = function(e) {
                    ...
                    //if default image - don't show preview.
                    if(imgSrc.indexOf('ignoreImages') > 0)
                    {
                        return;
                    }
                ...
                $scope.executeFuncOnRadioClick = function(row, col, radio) {
                    ...
                    if (typeof $scope.radioClickHandler == 'function') {
                        $scope.radioClickHandler(row, row.entity[col.field], col);
                    }
                };
				...
				var emptyLabel = {val: ""};
        $scope.selectNameFromId = function(col, row) {
					if (!row.entity._labels) row.entity._labels = {};
					var labels = row.entity._labels;
					if (labels[col.field] && labels[col.field].val) {
						return labels[col.field];
					} else {
						labels[col.field] = {isLoading: false};
					}

					var selectId = row.entity[col.field];
          var item = col.colDef;
          var dropdownList = [];
          ...
          if (typeof selectItem != "undefined") {
						labels[col.field].val = _.property('name')(selectItem);
						return labels[col.field];
          } else if (dropdownList && dropdownList.getList && !labels[col.field].isLoading) {
						labels[col.field].isLoading = true;
						dropdownList.getList({'q': selectId}).then(function(result) {
							if(result && result.length){
								result.forEach(function(item){
									if(item.id == selectId){
										labels[col.field].isLoading = false;
										labels[col.field].val = item.name;
									}
								});
							}
						});
