...
app.controller('placementEditCtrl', ['$scope', '$stateParams', 'mmRest', 'mmModal', 'mmAlertService', 'enums', '$state', 'mmUtils', 'entityMetaData',
  ...
  function ($scope, $stateParams, mmRest, mmModal, mmAlertService, enums, $state, mmUtils, entityMetaData,
            placementHelper, validationHelper, placementValidation, placementRest, tagGenerationService, servingAndCostService, placementJsonObjects, mmPermissions) {
    //var allSiteSections = null;
    var allPackages = null;
    //var siteSectionsIndex = {};
    var packageIndex = {};
    ...
    $scope.entityLayoutInfraButtons.saveButton = {name: 'save', func: save, ref: null, nodes: [], isPrimary: true};
		...
		$scope.trackingAdsTableColumns = [
			{field: 'id', displayName: 'Platform tracking ad ID', isColumnEdit: false, isShowToolTip: true, gridControlType: enums.gridControlType.getName("TextBox")},
			...
			{field: 'clickthrough.url', displayName: 'Click Through URL', validationFunction: validateClickThroughUrl,isColumnEdit: true, isShowToolTip: true, gridControlType: enums.gridControlType.getName("TextBox")},
			{field: 'thirdPartyImpressionTracker.url', displayName: '3rd Party impression tracker', validationFunction: validateThirdPartyImpressionUrl,isColumnEdit: true, cellEditableCondition: lockImpressionTrecking,isShowToolTip: true, gridControlType: enums.gridControlType.getName("TextBox")},
			{field: 'thirdPartyClickTracker.url', displayName: '3rd Party click tracker', validationFunction: validateThirdPartyClickUrl, isColumnEdit: true, isShowToolTip: true, gridControlType: enums.gridControlType.getName("TextBox")},
			{field: 'firstPartyAdName', displayName: '1st party ad Name', isColumnEdit: true, isShowToolTip: true, gridControlType: enums.gridControlType.getName("TextBox"), isPinned: true},
			{field: 'firstPartyAdId', displayName: '1st party ad ID', validationFunction: validateFirstPartyAdIdsUniqueness, isColumnEdit: true, isShowToolTip: true, gridControlType: enums.gridControlType.getName("TextBox")}
		];
    ...
    function processSites(data){
      ...
      if ($scope.$parent.mainEntity != null && $scope.isEditMode) {
        if (!$scope.$root.isDirtyEntity){
          $scope.editObject = $scope.$parent.mainEntity;
        }
...
//				mmAlertService.addError(error.data.error);
    ...
    //  tagGenerationService.openTagSettings($scope, [$scope.editObject]);
    //}
