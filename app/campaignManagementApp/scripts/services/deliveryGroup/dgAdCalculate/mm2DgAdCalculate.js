...
app.service('mm2DgAdCalculate', ['enums', 'dgConstants', 'dgAdCalculate', 'dgAdCalculateHelper','dgHelper',
    function (enums, dgConstants, dgAdCalculate, dgAdCalculateHelper,dgHelper) {
        var oneHundredPercent = 100;
        var actionOptions = dgAdCalculate.actionOptions;
        ...
        function calculateNewWeighted(subContainer, relative, newAd) {
            ...
            newAd.rotationSetting.weight = sumArr > 100 ? parseFloat(0) : dgAdCalculateHelper.formatNum(oneHundredPercent - sumArr, 0);
        ...
        function weightedEvenDistribution(container, isSecondGeneration) {
            ...
            var res = enabledSubContainers.length !== 0 ? dgAdCalculateHelper.formatNum(oneHundredPercent / enabledSubContainers.length, 0) : 0;
            ...
            for (var i = 0; i < subContainers.length; i++) {
                if (subContainers[i].type === dgConstants.strAdContainer) {
                    ...
                    weightedEvenDistribution(subContainers[i], true);
                }
        ...
        function updateSubContainersRotationType(actionType, container) {
            if (actionType == 'rotationChange') {
                ...
                for (var i = 0; i < stop; i++) {
                    ...
                    container.subContainers[i].rotationSetting.__type = dgConstants.mm2RotationSettingType[container.childRotationType];
        ...
        function weightedForNewAds(container) {
            ...
            for (var i = 0; i < oldArrLength; i++) {
                ...
                oldArr[i].rotationSetting.weight = (isNaN(num)) ? 0 : dgAdCalculateHelper.formatNum(num * relative, 0);
            }
        ...
        function weightedForEnableAds(container) {
            ...
            // calc weight for new item
            var res = (isWeightedExceedOneHundredPercent) ? 0 : dgAdCalculateHelper.formatNum((oneHundredPercent - sum) / totalNew, 0);
            for (var i = 0; i < totalNew; i++) {
                newItemsArr[i].rotationSetting.weight = res;
            }
        ...
        function calculateWeightedByAction(actionType, container, arrLengthBeforeChange) {
            switch (actionType) {
                case actionOptions.rotationChange:
                    weightedEvenDistribution(container, false);
                    break;
        ...
        function setSonsRotationOptions(container, parentRotationType) {
            if (dgHelper.isAdOrRootContainer(container)) {
                container.rotations = _.remove(angular.copy(enums.rotationSettingType) ,function(type){
                    return type.type != "TimeBased";
                });
