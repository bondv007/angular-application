...
app.service('calculateAdGroupRotation', ['enums', '$filter', 'deliveryGroupJsonObjects', 'dgConstants', 'dgHelper', function (enums, $filter, deliveryGroupJsonObjects, dgConstants, dgHelper) {
    ...
    function formatNum(num) {
        return num == 0 ? 0 : parseFloat($filter('number')(num, 2));
    }
    ...
    function calculateNewWeighted(subContainer, relative, newAd) {
        ...
        for (var i = 0; i < arrLength; i++) {
            ...
            subContainer[i].rotationSetting.weight = (isNaN(num)) ? 0 : formatNum(Math.round(relative * num));
        }
        var sumArr = sumArray(getArrOfWeights(subContainer));
        newAd.rotationSetting.weight = sumArr > 100 ? parseFloat(0) : formatNum(oneHundredPercent - sumArr);
        subContainer.push(newAd);
    }

    function calculateTotalWeighted(subContainer) {
        var arr = getArrOfWeights(subContainer);
        arr = _.compact(arr);
    ...
    function evenDistribution(container, isSecondGeneration) {
        ...
        var res = enabledSubContainers.length !== 0 ? formatNum(oneHundredPercent / enabledSubContainers.length) : 0;
    ...
    function calculateTimeBaseAfterAdd(container) {
        ...
        for (var i = 0; i < subContainers.length; i++) {
            if (subContainers[i].rotationSetting.type !== dgConstants.strTimeBased || !subContainers[i].rotationSetting.startDate) {
                ...
                subContainers[i].rotationSetting.rotationType = dgConstants.strTimeBased;
            }
        }
    }
    ...
    function setLastAdWeight(subContainers) {
        ...
        var sumArr = sumArray(getArrOfWeights(subContainers));
        ad.rotationSetting.weight = formatNum(oneHundredPercent - sumArr);
    ...
    function weightedForNewAds(container) {
        ...
        for (var i = 0; i < oldArrLength; i++) {
            ...
            oldArr[i].rotationSetting.weight = (isNaN(num)) ? 0 : formatNum(num * relative);
        }
        ...
        var res = (isWeightedExceedOneHundredPercent) ? 0 : formatNum((oneHundredPercent - sum) / totalNew);
    ...
    function calculateWeightedByAction(actionType, container, arrLengthBeforeChange) {
        ...
        container.totalWeight = formatNum(calculateTotalWeighted(container.subContainers));
        return container;
    }
