...
(function ($, window, undefined) {
	...
	var styles = {
		wrap: {
			...
			position: 'absolute', display: 'none', margin: 0, padding: 0, border: 'none','max-height': '650px', width: 'auto', height: 'auto', maxWidth: 'none', zIndex: -999999
		}
	};
...
//						bgCSS.top = '-' + bgOffset + 'px';
		...
		, show: function (index) {
			...
			// Vars
			var self = this
				, oldImage = self.$wrap.find(!isImage ? 'iframe' : 'img').addClass('deleteable')
			...
			// New image
			self.$img = $(!isImage ? '<iframe id="myFrame" auto-position-ad />' : '<img />')
				.css(styles.img)
