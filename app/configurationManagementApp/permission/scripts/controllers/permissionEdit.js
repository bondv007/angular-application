...
app.controller('permissionEditCtrl', ['$scope', '$rootScope', '$state', 'enums', '$stateParams', 'EC2Restangular', 'mmAlertService', 'mmPermissions', 'entityMetaData',
  function ($scope, $rootScope, $state, enums, $stateParams, EC2Restangular, mmAlertService, mmPermissions, entityMetaData) {
    ...
    function updateState() {
      if ($scope.$parent.mainEntity != null && $scope.isEditMode) {
        ...
        $scope.editPermission = mmPermissions.hasPermissionBySession(entityMetaData["permission"].permissions.entity.createEdit);
      } else {
