...
app.service('funnelService', ['unSavedChangesService', '$q', 'traffickingConst', function (unSavedChangesService, $q, traffickingConst) {
  ...
  function isLastAndCurrentSelectedTaTheSame(lastTASelected, targetAudienceCampaign, isDefaultTaSelected) {
    var isLastIsCurrentTa;
    if(isDefaultTaSelected){
      isLastIsCurrentTa = false;
    }
    else{
      isLastIsCurrentTa = lastTASelected && targetAudienceCampaign.targetAudienceId == lastTASelected.targetAudienceId;
    }
    return  isLastIsCurrentTa;
  }

  return {
    checkForUnSavedChanges: checkForUnSavedChanges,
    getDefaultTa: getDefaultTa,
    isLastAndCurrentSelectedTaTheSame: isLastAndCurrentSelectedTaTheSame
  };
}]);
