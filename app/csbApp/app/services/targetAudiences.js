app.factory( 'targetAudiences', [ '$q', 'csb', 'EC2Restangular',
	function( $q, csb, EC2Restangular ) {
		...
		pub.saveTargetAudiences = function( saveObject ) {
			...
			EC2Restangular.all( 'strategies' ).all( 'targetAudiences/' ).post( saveObject ).then(
				function( response ) {
					deferred.resolve({
						tas: response.plain(),
						error: ''
		...
		pub.updateTargetAudiences = function( updateObject ) {
			...
			EC2Restangular.all( 'strategies' ).all( 'targetAudiences/' ).customPUT( updateObject, null ).then(
				function( response ) {
					deferred.resolve({
						tas: response.plain(),
						error: ''
		...
		pub.deleteTargetAudience = function( id ) {
			...
			EC2Restangular.all( 'strategies' ).one( 'targetAudiences', id ).one( 'campaign', csb.params.campaignID ).remove().then(function( response ) {
