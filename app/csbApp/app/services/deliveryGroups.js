 ...
 */
'use strict';
app.factory( 'deliveryGroups', [ 'csb', 'EC2Restangular', '$q','mmAlertService','serverErrorMapper',
  function( csb, EC2Restangular, $q, mmAlertService, serverErrorMapper ) {
    var pub = {};
    var deliveryGroups = EC2Restangular.all('deliveryGroups/');

    pub.getCampaignDeliveryGroups = function () {
      var deferred = $q.defer();
      deliveryGroups.getList({"campaignId": csb.params.campaignID}).then(function (dgs) {
    ...
    pub.getTargetAudienceDeliveryGroups = function (targetAudienceId) {
      var deferred = $q.defer();
      deliveryGroups.getList({"campaignId": csb.params.campaignID, targetAudienceId: targetAudienceId}).then(function (dgs) {
        deferred.resolve(dgs);
      }, function () {
        mmAlertService.addError("Server error. Please try again later");
        deferred.reject();
